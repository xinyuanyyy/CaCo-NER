...
from .tester import Tester,Tester_finetune_ctr
...
class Trainer(object):
    ...
    def __init__(self, train_data, model, optimizer=None, loss=None,
                 ...
                 dev_data=None, test_data=None, metrics=None, metric_key=None,
                 validate_every=-1, save_path=None, use_tqdm=True, device=None,
                 callbacks=None, check_code_level=0, is_ctr=False, train_writer=None, max_patience=25,args=None,params=None,label_size=None,finetune=False,train_prop=1,**kwargs):
        """
        ...
        if isinstance(train_data, DataSet):
            self.data_iterator = DataSetIter(
                dataset=train_data[:int(train_prop*len(train_data))], batch_size=batch_size, num_workers=num_workers, sampler=sampler,
                drop_last=drop_last)
        ...
        if _model_contains_inner_module(self.model):
            ...
            self._forward_func = self.model.forward  # model是传进来的参数，这里把model的forward函数传给_forward_func
        ...
        self.kwargs = kwargs
        # self.recover_path=os.path.join(self.output_dir, "ctr_recover")
    ...
    def train(self, load_best_model=True, on_exception='auto'):
        ...
        try:
            ...
            if self.dev_data is not None and self.best_dev_perf is not None:
                ...
                self.logger.info(self.tester._format_eval_results2(self.best_dev_perf))
                results['best_eval'] = self.best_dev_perf
                results['best_epoch'] = self.best_dev_epoch
                results['best_step'] = self.best_dev_step

                best_test_metric=self.testDict[self.best_dev_epoch]
                if self.is_ctr and self.finetune:
                    self.model.load_state_dict(torch.load(self.best_state_path, map_location=self.device))
                    self.finetuner.train()
                    result_test = self.tester_test.test()
                    test = result_test['SpanFPreRecMetric']
                    for k,v in test.items():
                        best_test_metric[k]=v
                    # os.remove(self.recover_path)
                # return best_test_metric, self.best_state_path, self.best_dev_epoch
                return best_test_metric, self.best_state_path, self.best_dev_epoch,results
                # if load_best_model:
                #     model_name = "best_" + "_".join([self.model.__class__.__name__, self.metric_key, self.start_time])
                #     load_succeed = self._load_model(self.model, model_name)
                #     if load_succeed:
                #         self.logger.info("Reloaded the best model.")
                #         return self.testList[self.best_dev_epoch-1],self.best_state_path #os.path.join(self.save_path, model_name)
                #     else:
    ...
    def save_model(self, model, modelname, BertBaseName, output_dir, best_epoch, best_f1):
        model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
        saving_path = os.path.join(output_dir,
                                   modelname + "_" + BertBaseName + "_e_" + str(best_epoch) + "_f_" + str(
                                       best_f1) + ".bin")
        torch.save(model_to_save.state_dict(), saving_path)
        ...
        with inner_tqdm(total=self.n_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True) as pbar:
            ...
            for epoch in range(1, self.n_epochs + 1):
                ...
                for batch_x, batch_y in data_iterator:
                    ...
                    loss = self._compute_loss(prediction, batch_y).mean()  # 这个loss是假的，用的其实就是crf内最后差值
                    avg_loss += loss.item()
                    ...
                    if ((self.validate_every > 0 and self.step % self.validate_every == 0) or
                        ...
                        eval_results, test_results = self._do_validation(epoch=epoch, step=self.step)

                        writer.write("----- Training epoch" + str(epoch) + " -----\n")
                        writer.write("%s = %s\n" % ('loss', str(total_loss/step_in_epoch)))
                        writer.write("***** Eval results *****\n")
                        for key in sorted(eval_results.keys()):
                            writer.write("%s = %s\n" % (key, str(eval_results[key])))
                        # test_results=self.testers['data-test'].test()['SpanFPreRecMetric']
                        writer.write("***** Test results *****\n")
                        self.testDict[self.epoch]=test_results
                        for key in sorted(test_results.keys()):
                            writer.write("%s = %s\n" % (key, str(test_results[key])))

                        writer.flush()
                        if self.is_ctr:
                            primary_key='loss'
                            better_than = lambda x,y: x -y<0
                        else:
                            primary_key='f'
                            better_than = lambda x,y: x - y > 0

                        if better_than(eval_results[primary_key] , best_metric[primary_key]):
                            if self.best_state_path:
                                os.remove(self.best_state_path)
                            best_epoch = epoch
                            self.best_state_path = self.save_model(self.model, "Flat" + '_eval',
                                                                   "bert-base-chinese",
                                                                   self.save_path, best_epoch, eval_results[primary_key])
                            best_metric = eval_results
                            current_patience = 0
                            self.best_dev_epoch = best_epoch
                            self.best_dev_perf = best_metric
                            ...
                            if self.max_patience <= current_patience:
                                print('finished training! (early stopping, max patience: {0})'.format(
                                    self.max_patience))
                                finish_flag = True
                        writer.flush()
                        ...
                        # self.logger.info(eval_str)
                        self.logger.info(self.tester._format_eval_results2(eval_results) + '\n')
                if finish_flag:
                    break
                # ================= mini-batch end ==================== #
    ...
    def _do_validation(self, epoch, step):
        ...
        if self.is_ctr:
            # torch.save(self.model.state_dict(),self.recover_path)
            dev = self.tester.test2() #test2 is for computing ctr loss
            test = self.tester_test.test2()
            ...
            result_test = self.tester_test.test()
            dev=res['SpanFPreRecMetric']
            dev['loss']=float(res['loss'])
            test=result_test['SpanFPreRecMetric']
            test['loss'] = float(result_test['loss'])
            # self.model.load_state_dict(torch.load(self.recover_path, map_location=self.device))
        return (dev, test)
    ...
    def _data_forward(self, network, x,**kwargs):
        x = _build_args(self._forward_func, **x,**kwargs)
        y = network(**x)
