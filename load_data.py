...
def load_ontonotes4ner(path,char_embedding_path=None,bigram_embedding_path=None,index_token=True,train_clip=False,
    ...
    train_path = os.path.join(path,'train.txt{}'.format('_clip' if train_clip else ''))
...
@cache_results(_cache_fp='cache/resumeNER_uni+bi', _refresh=False)
def load_resume_ner(path,unigram_embedding_path=None,bigram_embedding_path=None,index_token=True,
    ...
    train_path = os.path.join(path,'train.txt') #truncated_
    dev_path = os.path.join(path, 'dev.txt')#truncated_
    test_path = os.path.join(path, 'test.txt')#truncated_

    # train_path = os.path.join(path,'train.txt') #full
    # dev_path = os.path.join(path, 'dev.txt')
    # test_path = os.path.join(path, 'test.txt')
...
@cache_results(_cache_fp='cache/resume_ner',_refresh=True)
def load_resume_ner_old(path,char_embedding_path=None,bigram_embedding_path=None,index_token=True,
                    char_min_freq=1,bigram_min_freq=1,only_train_min_freq=0):
    from fastNLP.io.loader import ConllLoader
    from utils import get_bigrams
...
def load_yangjie_rich_pretrain_word_list(embedding_path,drop_characters=True):
    ...
    if drop_characters:
        w_list = list(filter(lambda x:len(x) != 1, w_list)) # 698668 after filter
...
def load_msra_ner_1(path,char_embedding_path=None,bigram_embedding_path=None,index_token=True,train_clip=False,
    ...
    if train_clip:
        train_path = os.path.join(path, 'train_bmes.txt_clip2.bio.txt') #train_dev.char.bmes_clip2
        print(train_path)
        test_path = os.path.join(path, 'test_bmes.txt_clip2.bio.txt')
    # if train_clip:
    #     train_path = os.path.join(path, 'train.txt_clip2') #---250, max_length
    #     print(train_path)
    #     test_path = os.path.join(path, 'test.txt_clip2')
    else:
...
@cache_results(_cache_fp='cache/imcs_ner',_refresh=False)
def load_imcs_ner(path,unigram_embedding_path=None,bigram_embedding_path=None,index_token=True,
                    char_min_freq=1,bigram_min_freq=1,only_train_min_freq=0,char_word_dropout=0.01):
    from fastNLP.io.loader import ConllLoader
    from utils import get_bigrams
...
@cache_results(_cache_fp='cache/cmeeev2_ner',_refresh=False)
def load_cmeeev2_ner(path,unigram_embedding_path=None,bigram_embedding_path=None,index_token=True,
                     char_min_freq=1,bigram_min_freq=1,only_train_min_freq=0,char_word_dropout=0.01):
    from fastNLP.io.loader import ConllLoader
    from utils import get_bigrams
